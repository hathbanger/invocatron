...
InvocaBot.prototype._replyWithRandomJoke = function (originalMessage) {
    ...
    function recursive_invoca_request(date, lastElId, count){
        ...
        if(lastElId == ''){
            var r = request('https://connectyourhome.invoca.net/api/2016-10-01/networks/transactions/1219.json?from='+date+'&to='+date+'&limit=4000&&include_columns=transaction_id&oauth_token='+oauth, function(err, res, body){
                var response = JSON.parse(body);
                resLen += response.length;
                
                if(resLen % 4000 == 0){
                    console.log("divisible by 4k!");
                    var lastId = response[resLen - 1].transaction_id;
                    var midStr = "Chuck thinks there's more records. He's going back in. He's gonna start from record id: " + lastId;
                    self.postMessageToChannel(channel.name, midStr, {as_user: true});                    
                    recursive_invoca_request(date, lastId, resLen)
                } else {
                    console.log('wasnt divisble by 4k');
                    self.postMessageToChannel(channel.name, "BOOM! Ya BOY CHUCK is back, w/ *" + resLen + "* records!", {as_user: true});
                }

                var resStr = 'Chuck Norris wrestled '+resLen+' records out of Invoca\'s database. huzzah!';
                self.postMessageToChannel(channel.name, resStr, {as_user: true});
            });
        } else {
                request('https://connectyourhome.invoca.net/api/2016-10-01/networks/transactions/1219.json?from='+date+'&to='+date+'&limit=4000&include_columns=transaction_id&start_after_transaction_id='+ lastElId +'&oauth_token='+oauth, function(err, res, body){
                var response = JSON.parse(body);
                resLen += response.length;
                ...
                if(resLen % 4000 == 0){
                    console.log("divisible by 4k!");
                    console.log(resLen)
                    console.log(response[3999]);
                    var lastId = response[3999].transaction_id;
                    var midStr = "Chuck makes *_BIG_* bucks retrieving invoca records.";
                    self.postMessageToChannel(channel.name, midStr, {as_user: true});                    
                    recursive_invoca_request(date, lastId, resLen)
                } else {
                    console.log('wasnt divisble by 4k');
    ...
    recursive_invoca_request('2017-03-27', '', 0);
};
